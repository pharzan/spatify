...
export interface SpatiImageUpload {
  stream: NodeJS.ReadableStream;
  filename: string;
  mimetype: string;
}

export interface SpatiImageStorage {
  upload(image: SpatiImageUpload): Promise<string>;
  delete(imageUrl: string): Promise<void>;
}

export class GcsSpatiImageStorage implements SpatiImageStorage {
  ...
  async upload(image: SpatiImageUpload): Promise<string> {
  ...
  private buildObjectName(originalFilename: string): string {
    ...
    const extension =
      safeName.includes('.') && safeName.lastIndexOf('.') > 0
        ...
        : '';
    return `spatis/${randomUUID()}${extension.toLowerCase()}`;
  }
